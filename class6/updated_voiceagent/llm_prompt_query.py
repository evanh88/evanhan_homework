...
def build_prompt(st_chat_messages):
    ...
    if len(st.session_state.messages) >= 3:
        last_user_msg = st_chat_messages[-3]["text"]
        last_assistant_msg = st_chat_messages[-2]["text"]
        ...
        for user_msg, assistant_msg in recent_turns:
            ...
            prompt += EOTN
    prompt += format_turn("user", st_chat_messages[-1]["text"])

    prompt += EOTN
    prompt += f"{SOF}assistant{EOF}"  # Assistant will continue from here
    return prompt

def query_llm(st_chat_messages):            
    # Construct the full prompt
    prompt = build_prompt(st_chat_messages)
